...
export class SamlStrategy extends PassportStrategy(Strategy, 'saml') {
  ...
  async validate(profile: any): Promise<UserPayload> {
    // Extract Tenant ID from NameID or Attributes (Custom SAML mapping)
    const email = profile.nameID;
    const tenantSlug =
      profile?.attributes?.['http://schemas.meru.com/identity/tenant'];

    if (!tenantSlug)
      throw new UnauthorizedException('SAML missing tenant context');
